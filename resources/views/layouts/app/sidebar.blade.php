        ...
        <flux:sidebar.nav>
            ...
            @if ($canSeeAdquisicionesGroup && ($isAdmin || ($modulos['adquisiciones'] ?? false)))
                <flux:sidebar.group :heading="__('Adquisiciones e Inventario')" class="grid mt-4">
                    ...
                    @if (auth()->user()->hasRole(['administrador', 'superadmin']) || auth()->user()->can('aprobar-requerimientos'))
                    ...
                    @if (auth()->user()->hasRole(['administrador', 'superadmin', 'ti']) || auth()->user()->can('ver-prestamos-general'))
