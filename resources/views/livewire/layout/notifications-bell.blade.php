...
new class extends Component {
    ...
    /**
     * Initialize the component and pre-populate notified IDs to prevent toast deluge on initial page load.
     */
    public function mount(): void
    {
        if (Auth::check()) {
            $this->notifiedIds = Auth::user()->unreadNotifications->pluck('id')->toArray();
        }
    }

    /**
     * Mark a specific notification as read and redirect if a URL is provided.
     */
    public function markAsRead(string $id, ?string $url = null)
    {
        if (Auth::check()) {
    ...
    /**
     * Check for any newly arrived notifications in real-time to trigger Flux toasts.
     * Triggered via polling wire:poll.15s="checkNewNotifications"
     */
    public function checkNewNotifications(): void
    {
        if (Auth::check()) {
            ...
            foreach ($unread as $notification) {
                if (!in_array($notification->id, $this->notifiedIds)) {
                    // Map notification custom color to corresponding Flux toast variant
                    ...
                    // Trigger elegant real-time desktop toast alert
                    \Flux::toast(
                        heading: $notification->data['titulo'] ?? __('Nueva Notificación'),
                        text: $notification->data['mensaje'] ?? '',
                        variant: $variant
                    );

                    // Add to notified tracking list
                    $this->notifiedIds[] = $notification->id;
                }
            }
        }
    }
