...
new class extends Component {
    ...
    public function mount()
    {
        ...
        $this->kpiMes = Entrevista::where('school_id', $schoolId)
        ...
        // Motivos (del mes)
        $mots = Entrevista::where('school_id', $schoolId)
            ->whereBetween('fecha', [$mesInicio, $mesFin])
...
<div class="max-w-7xl mx-auto w-full pb-10">
    ...
    <x-header titulo="Dashboard Entrevistas"
        subtitulo="Monitoreo de gestión de entrevistas y rendimiento a nivel institucional." icono="chart-bar">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <!-- KPI 1: Hoy -->
        <a href="{{ route('entrevistas.index', ['fecha' => now('America/Santiago')->format('Y-m-d'), 'filtroTemporal' => 'dia']) }}"
            ...
            <flux:card class="border-t-4 border-t-blue-500 hover:shadow-md transition-shadow cursor-pointer h-full">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <span
                        class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{{ __('Hoy') }}</span>
                </div>
                ...
                <p class="text-4xl font-extrabold text-zinc-900 dark:text-white mt-1">
                    {{ str_pad($kpiHoy, 2, '0', STR_PAD_LEFT) }}</p>
            </flux:card>
        </a>
        ...
        <a href="{{ route('entrevistas.index', ['estado' => 'ingresada']) }}" class="block no-underline">
            <flux:card class="border-t-4 border-t-amber-500 hover:shadow-md transition-shadow cursor-pointer h-full">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <span
                        class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{{ __('Activo') }}</span>
                </div>
                <p class="text-zinc-500 dark:text-zinc-400 text-xs font-bold uppercase">{{ __('En Proceso / Espera') }}
                </p>
                <p class="text-4xl font-extrabold text-zinc-900 dark:text-white mt-1">
                    {{ str_pad($kpiActivos, 2, '0', STR_PAD_LEFT) }}</p>
            </flux:card>
        </a>
                ...
                @forelse($topDocentes as $index => $docente)
                    <div class="space-y-2">
                        <div class="flex justify-between text-sm font-semibold mb-1">
                            <span class="text-zinc-700 dark:text-zinc-300">{{ $docente->nombres }}
                                {{ $docente->apellido_pat }}</span>
                        ...
                        <div class="w-full bg-zinc-100 dark:bg-zinc-800 h-2.5 rounded-full overflow-hidden">
                            ...
                            @endphp
                            <div class="bg-blue-500 dark:bg-blue-400 h-full rounded-full transition-all duration-1000"
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            <span
                                class="text-sm font-bold text-zinc-900 dark:text-white">{{ $urgencias['normal']['count'] }}</span>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            <span
                                class="text-sm font-bold text-zinc-900 dark:text-white">{{ $urgencias['prioritario']['count'] }}</span>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="text-right">
                            <span
                                class="text-sm font-bold text-zinc-900 dark:text-white">{{ $urgencias['urgente']['count'] }}</span>
                        ...
                        <div class="md:col-span-6 flex justify-center items-center">
                            <div class="relative w-36 h-36" x-init="window.initMotivosChart ? window.initMotivosChart() : document.addEventListener('livewire:navigated', () => window.initMotivosChart())">
                                ...
                                <div
                                    ...
                                    <span
                                        class="text-2xl font-extrabold text-zinc-900 dark:text-white">{{ array_sum(array_column($motivos, 'count')) }}</span>
                                    <span
                                        class="text-[8px] uppercase tracking-widest font-bold text-zinc-400 mt-0.5">{{ __('Citas') }}</span>
                                </div>
                            </div>
                        </div>
                            ...
                            @foreach (array_slice($motivos, 0, 7) as $motivo => $data)
                                <div class="flex items-center justify-between text-xs">
                                    <div class="flex items-center gap-2 min-w-0">
                                        <div
                                            class="w-2.5 h-2.5 rounded-full shrink-0 {{ $colors[$loop->index] ?? 'bg-zinc-500' }}">
                                        </div>
                                        <span class="font-semibold text-zinc-700 dark:text-zinc-300 truncate"
                                            title="{{ ucfirst($motivo) }}">{{ ucfirst($motivo) }}</span>
                                    </div>
                                    <div class="text-right shrink-0">
                                        <span
                                            class="font-bold text-zinc-900 dark:text-white ml-1.5">{{ $data['count'] }}</span>
