...
<div>
    <div class="flex flex-col gap-8 w-full max-w-7xl mx-auto">
        <!-- Quick Action Header -->
        <div>
            <x-header :titulo="__('Listado de Funcionarios')" :subtitulo="__(
                'Administración centralizada de personal docente, administrativo y auxiliar del establecimiento.',
                            ...
                            <flux:table.cell>
                                <div class="flex flex-wrap gap-1">
                                    ...
                                    @forelse ($funcionario->active_roles as $role)
                                        @php
                                            $roleData = $roleLabels[$role] ?? [
                                                'label' => ucfirst($role),
                                                'color' => 'zinc',
                                            ];
                                        @endphp
                                        <flux:badge :color="$roleData['color']">{{ __($roleData['label']) }}
                                        </flux:badge>
                                    @empty
                            ...
                            <flux:table.cell class="text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <flux:button variant="ghost" size="sm" icon="eye"
                                        ...
                                        :href="route('funcionarios.ficha', $funcionario->id)" />
                                    <flux:button variant="ghost" size="sm" icon="pencil-square"
                                        :tooltip="__('Editar')" wire:click="abrirEditar({{ $funcionario->id }})" />
                                    <flux:button variant="ghost" size="sm" icon="trash"
                                        :tooltip="__('Eliminar')"
                                        wire:click="confirmarEliminar({{ $funcionario->id }})" />
                                </div>
                            </flux:table.cell>
                        </flux:table.row>
                    @endforeach
    ...
    <flux:modal wire:model="modalAbierto" class="md:w-xl">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ $funcionarioId ? __('Editar Funcionario') : __('Nuevo Funcionario') }}
                </flux:heading>
                <flux:text class="mt-2">
                    {{ $funcionarioId ? __('Modifica los datos del funcionario.') : __('Ingresa los datos del nuevo funcionario.') }}
                </flux:text>
            </div>
    ...
    <flux:modal wire:model="modalEliminar" class="md:w-80">
        <div class="space-y-6">
            <div>
                ...
                <flux:text class="mt-2">
                    {{ __('El funcionario será desvinculado del colegio. Esta acción no puede deshacerse.') }}
                </flux:text>
            </div>
            ...
            <div class="flex">
                ...
                <flux:button wire:click="eliminar" variant="danger" class="ml-2">{{ __('Eliminar') }}
                </flux:button>
            </div>
        </div>
    </flux:modal>
